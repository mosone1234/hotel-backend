...
const findAll = async (req, res) => {
  try {
    const response = await PersonaCuarto.findAll();
...
const create = async (req, res) => {
  ...
  try {
    const response = await PersonaCuarto.create(data);
...
const update = async (req, res) => {
  const data = Object.assign({}, req.body);
  try {
    const response = await PersonaCuarto.findOne({ where: { id: data.id } });
    if (response === null) {
      throw "El cliente no existe!";
    } else {
      try {
        await PersonaCuarto.update(data, {
          where: { id: data.id },
        });
...
const findById = async (req, res) => {
  ...
  try {
    const response = await PersonaCuarto.findOne({ where: { id: id } });
    if (response === null) {
      throw "El cliente no existe!";
    } else {
      res.status(200).json(response);
...
const destroy = async (req, res) => {
  const id = req.params.id;
  try {
    const response = await PersonaCuarto.destroy({ where: { id: id } });
