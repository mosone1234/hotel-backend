...
const findAll = async (req, res) => {
  try {
    const response = await Cuarto.findAll();
...
const create = async (req, res) => {
  ...
  try {
    const response = await Cuarto.create(data);
...
const update = async (req, res) => {
  const data = Object.assign({}, req.body);
  try {
    const response = await Cuarto.findOne({ where: { id: data.id } });
    if (response === null) {
      throw "El cliente no existe!";
    } else {
      try {
        await Cuarto.update(data, {
          where: { id: data.id },
        });
...
const findById = async (req, res) => {
  ...
  try {
    const response = await Cuarto.findOne({ where: { id: id } });
    if (response === null) {
      throw "El cliente no existe!";
    } else {
      res.status(200).json(response);
...
const destroy = async (req, res) => {
  const id = req.params.id;
  try {
    const response = await Cuarto.destroy({ where: { id: id } });
